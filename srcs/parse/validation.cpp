...
void	validate_tokens(std::vector<BaseAssignmentType *> tokens, bool is_compute_action)
{
	...
	{
		if (tokens.size() > 2 && tokens[1]->getType() != OPERATOR_EQ) throw Ft_error("validate_tokens: Can only have 1 term before '='");
	}

	// must not have more than 1 '='
	if (std::count_if(tokens.begin(), tokens.end(), is_eq) > 1) throw Ft_error("validate_tokens: Can only have 1 '='");
	...
	for (std::vector<BaseAssignmentType *>::iterator i = tokens.begin(); i != tokens.end(); i++)
	{
		...
		if ((i != tokens.begin()) && i != tokens.end() - 1 && is_operator(curr_token) && (is_operator(*(i-1)) || is_operator(*(i+1))))
				throw Ft_error("validate_tokens: Operators cant be next to each other");
		...
		if (is_operator(curr_token))
		{
			if (i != tokens.end() - 1)
			{
				...
				if ((*(i + 1))->getType() == R_PARENTHESIS) throw Ft_error("validate_tokens: Right of operator cant be R_PARENTHESES");
			}
			if (i != tokens.begin())
			{
				// operator cant be nxt to qmark 
				...
				if ((*(i - 1))->getType() == L_PARENTHESIS) throw Ft_error("validate_tokens: Left of operator cant be L_PARENTHESES");
			}

		}

		// operators cant be at the end 
		if (is_operator(curr_token) && i == tokens.end() - 1) throw Ft_error("validate_tokens: Term expected after operator");

		// no variables should have name i
		if (curr_token->getType() == VAR && curr_token->toString() == "i") throw Ft_error("validate_tokens: Variable name 'i' is forbidden");
		...
		{
			if (parenthesis_stk_cnt == 0) throw Ft_error("validate_tokens: Bad parenthesis");
			--parenthesis_stk_cnt;
		}
	}

	// if parenthesis does not match, throw err
	if (parenthesis_stk_cnt != 0) throw Ft_error("validate_tokens: Parenthesis does not match");
}